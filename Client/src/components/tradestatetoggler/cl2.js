...
class TradeState extends React.Component {
  ...
  toggle=(algo,client)=>{
    this.setState({showModal:true,algoname:algo,
      clientid:client })
  }
      ...
      <Fragment>
        <Sticky>
          <Menu style={{ margin: 0 }}>
            ...
            <Link to="/d7730002status">
              <Menu.Item active={true}>D7730002</Menu.Item>
            </Link>
        ...
        <table className="ui celled table">
          ...
          <tbody>
            {this.props.data.map((row) => {
              ...
              if (row.ClientID === "D7730002" && row.Start_Stop !== "STOP")
                return (
                  <tr key={row._id}>
                    <td>{row.ClientID}</td>
                    <td>{row.algoname}</td>
