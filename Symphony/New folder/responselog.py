...
raw_dict = {}
raw_response_dict = {}
old_value_count = 0# raw_db[collec].count()
oldresponse_count = 0#responsedb[responsedb_collec].count()
...
while True:
	...
	# if list_unique:
	# 	for unqID in list_unique:
	# 		cancellation_status = responsedb[responsedb_collec].find(
	# 			{"OrderUniqueIdentifier": unqID})
	# 		for cancelled in cancellation_status:
	# 			if cancelled['OrderStatus'] == "Cancelled":
	# 				cancelled_post = {
	# 					'quantity': raw_dict[unqID]['quantity'],
	# 					'algoname': raw_dict[unqID]['algoname'],
	# 					'instrumentID': symbol,
	# 					'orderside': raw_dict[unqID]['orderside'],
	# 					'timestamp': raw_response_dict[unqID]['timestamp'],
	# 					'orderstatus': raw_response_dict[unqID]['orderstatus'],
	# 					'cancelrejectreason': raw_response_dict[unqID]['cancelrejectreason'],
	# 					# 'rejectedquantity': raw_response_dict[unqID]['rejectedquantity']
	# 					'OrderAverageTradedPrice': raw_response_dict[orderid]['OrderAverageTradedPrice'],
	# 					'ClientID': raw_response_dict[orderid]['ClientID']
	# 				}
	# 				final_response[final_response_collec].insert_one(cancelled_post)
	# 				list_unique.remove(unqID)
				...
				for orderid in raw_dict.keys():
					...
					if str(orderid) in raw_response_dict.keys():
						...
						if raw_response_dict[orderid]['orderstatus'] == "Filled" or raw_response_dict[orderid]['orderstatus'] == "Rejected" or raw_response_dict[orderid]['orderstatus'] == "Cancelled":
							...
							print(final_post)
							final_response[final_response_collec].insert_one(final_post)
