...
def savedata(post):
    try:
        ...
        new_match = match = db[collec].find_one(
            {"$and": [{"clientID": post['clientID']}, {"symbol": post['symbol']}]})
        if new_match:
            # print(new_match)
            if(new_match["quantity"] != post["quantity"]):
                db[collec].update({'_id': new_match['_id']}, {
        ...
        try:
            for client_id in client:
                ...
                if new_client:
                    ...
                    match = cumulative_db[cumulative_collec].aggregate([{"$match": {"clientID": client_id}}, {
