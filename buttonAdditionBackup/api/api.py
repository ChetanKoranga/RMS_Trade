...
def checkPostedData(postedData, method):
    ...
    if method == "finishupdate":
        if "Status" not in postedData:
            return 301
        else:
            return 200
...
class finishupdate(Resource):
    ...
    def post(self):
        postedData = request.get_json()
        finish = postedData["Status"]
        status = checkPostedData(postedData, "finishupdate")
        if status == 301:
            return "Parameter error. Please check the parameters"
        elif status == 200:
            if finish == "finishinfraday":
                match1 = db[collec].find({"algoname": {"$regex": '^(?!2).*'}})
                for i in match1:
                    db[collec].update_one({'_id': i['_id']}, {
                                          "$set": {"Start_Stop": "FINISH"}})

            elif finish == "finishovernight":
                match2 = db[collec].find({"algoname": {"$regex": '^2'}})
                for i in match2:
                    db[collec].update_one({'_id': i['_id']}, {
                                          "$set": {"Start_Stop": "FINISH"}})
            elif finish == "restartinfraday":
                matchre = db[collec].find({"algoname": {"$regex": '^(?!2).*'}})
                for i in matchre:
                    db[collec].update_one({'_id': i['_id']}, {
                                          "$set": {"Start_Stop": "START"}})
            elif finish == "restartovernight":
                matchover = db[collec].find({"algoname": {"$regex": '^2'}})
                for i in matchover:
                    db[collec].update_one({'_id': i['_id']}, {
                                          "$set": {"Start_Stop": "START"}})
...
api.add_resource(finishupdate, "/finishupdate")
app.run(debug=True)
