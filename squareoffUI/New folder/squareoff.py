...
class Toplevel1:
        ...
        for doc in documents:
            ...
            if symbol.startswith("BANKNIFTY") and orderQuantity != 0:
                if abs(orderQuantity) > 60:
                    p1 = Process(target=obj.place_order, args=(
                        exchangeSegment, exchangeInstrumentID, productType, orderType, orderSide, abs(
                            orderQuantity), algoName, clientID, 60,
                        20, 0.1)).start()
                else:
                ...
                if abs(orderQuantity) > 225:
                    p1 = Process(target=obj.place_order, args=(
                        exchangeSegment, exchangeInstrumentID, productType, orderType, orderSide, abs(
                            orderQuantity), algoName, clientID, 225,
                        75, 0.2)).start()
                else:
                    order.place_order(exchangeSegment, exchangeInstrumentID, productType, orderType, orderSide,
                                      abs(orderQuantity), algoName, clientID)
    ...
    def __init__(self, top=None):
        ...
        with open('strategylist.txt') as csv_file:
            csv_reader = csv.reader(csv_file, delimiter=',')
            strategy_list = []
            for row in csv_reader:
                # print(row)
                if row:
                    strategy_list.append(row[0])
                
            print(strategy_list)
            # res = str(strategy_list)[1:-1]
            # print(type(res))
            # print(res)

        self.textstrategy = OptionMenu(self.Frame2, self.var_strategy,*strategy_list)
        ...
        with open('clientlist.txt') as csv_file:
            csv_reader = csv.reader(csv_file, delimiter=',')
            client_list = []
            for row in csv_reader:
                # print(row)
        ...
        self.textclient = OptionMenu(self.Frame2, self.var_client,*client_list)
        # self.textalgoname.place(relx=0.142, rely=0.05, x=-2, y=2)
