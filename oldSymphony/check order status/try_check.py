...
def sound():
    ...
    if(status == "Rejected"):
        alarm_reason("6","error","position check","server system","position is rejected")
        # alarm_reason("alarm-sound (5).mp3","position check","server system","position is rejected")
        playsound.playsound("alarm-sound (5).mp3")
        time.sleep(0.5)
    elif(status == "Cancelled"):
        alarm_reason("7","error","position check","server system","position is cancelled")
